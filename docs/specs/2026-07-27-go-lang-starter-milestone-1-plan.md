...
| I15 `[TDD]` MFA completion | `api/internal/modules/identity/application/mfa/verify_test.go` | `api/internal/modules/identity/application/mfa/verify.go` | one-time/recovery code; conditional revoke pending + insert authenticated row ใหม่ใน transaction เดียว; ห้าม update token/state บนแถวเดิม |
