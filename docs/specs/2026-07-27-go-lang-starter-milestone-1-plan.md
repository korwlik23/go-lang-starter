...
| M13 | `api/tests/migrations/system_role_assignments{,_behavior,_consistency}_test.go` | `api/internal/modules/authorization/migrations/{dialect}/000005_create_system_role_assignments.sql` | exact 4-column user/role composite PK; exact binary `system` discriminator + composite role-owner FK ทำให้ account role อ้างไม่ได้, user FK update-restrict/delete-cascade, role FK restrictive และมี owner-role support + reverse `(role_id, user_id)` indexes; ไม่มี seed/account snapshot |
...
| O3L `[TDD]` locale-preference contract | `api/tests/contract/openapi_test.go` | `api/openapi/modules/localization/locales.yaml` | RED preferences operation absent; GREEN request รับ `expected_version` โดย first write ใช้ 0, subset locale IDs เป็น unique `maxItems: 256` ภายใต้ operation body cap 32 KiB และ response คืน version ใหม่; mismatch/concurrent first insert คืน stable conflict โดยไม่มี side effect |
...
| Q9A `[TDD]` safe role assignment | `api/internal/modules/authorization/application/assign_role_test.go` | `api/internal/modules/authorization/application/assign_role.go` | account path lock role+membership และ system path lock role+active target user แล้ว pin persisted mappings ทั้งหมด; reject role ที่มี inactive/non-delegable/ผิด scope/cross-owner mapping, revalidate assigner operation+exact target permissions, role version และ catalog epoch ตอน commit; system path รับเฉพาะ active `system` permissions และทุก path มี concurrent tests |
| Q10 `[TDD]` last manager | `api/internal/modules/authorization/application/revoke_role_test.go` | `api/internal/modules/authorization/application/revoke_role.go` | ปฏิเสธการลบ/revoke/deactivate manager คนสุดท้ายตาม effective permission set ไม่ใช่ชื่อ role ด้วย account-scoped หรือ global system-scoped lock/CAS ตาม principal; ครอบคลุม assignment, role-permission และ user/membership lifecycle โดย concurrent revoke มีเพียงหนึ่ง transaction commit |
...
| ID | Test file | Implementation files | Behavior |
|---|---|---|---|
| L1 `[TDD]` locale tag/registry | `api/internal/modules/localization/domain/locale_test.go` | `api/internal/modules/localization/domain/locale.go`, `api/internal/modules/localization/application/list_locales.go` | arbitrary valid BCP 47 tags normalize เป็น lowercase internal identity โดยไม่จำกัด allowlist/จำนวนภาษา, one initialized default และ enabled/user_selectable แยกกัน |
| L1A `[TDD]` locale persistence | `api/internal/modules/localization/adapters/gorm/locale_repository_test.go` | `api/internal/modules/localization/ports/locale_repository.go`, `api/internal/modules/localization/adapters/gorm/locale_repository.go` | UUID/tag immutable, normalized tag uniqueness, transactional single-default/fallback-root transfer ด้วย optimistic version + stable registry serialization, fallback target ต้อง enabled และกราฟต้อง acyclic/bounded; list enabled/selectable/default แบบ deterministic |
| L1B `[TDD]` manage locales | `api/internal/modules/localization/application/manage_locale_test.go` | `api/internal/modules/localization/application/create_locale.go`, `api/internal/modules/localization/application/update_locale.go`, `api/internal/modules/localization/application/set_default_locale.go` | create/update/enable/user-selectable/default ต้องมี `localization.locales.manage.system`; account permission deny, default ห้าม disable และ lifecycle ใช้ disable แทน generic hard-delete |
...
| L3 `[TDD]` merge order | `api/internal/modules/localization/application/catalog/merge_test.go` | `api/internal/modules/localization/application/catalog/merge.go`, `api/internal/modules/localization/ports/catalog_repository.go` | DB override > bundle > bounded acyclic per-locale UI fallback chain; fallback locale ต้อง enabled และ zero/default-cycle state fail closed |
| L3A `[TDD]` catalog persistence | `api/internal/modules/localization/adapters/gorm/catalog_repository_test.go` | `api/internal/modules/localization/adapters/gorm/catalog_repository.go` | locale/category/key immutable identity; transactional CAS upsert/reset โดย reset เขียน `value=NULL` tombstone และ version เดินต่อเนื่องเพื่อกัน ABA, ไม่มี generic hard-delete; active projection/deterministic export ใช้เฉพาะ non-NULL values |
...
| L5 `[TDD]` selectable subset | `api/internal/modules/localization/application/preferences/update_test.go` | `api/internal/modules/localization/application/preferences/update.go`, `api/internal/modules/localization/ports/preference_repository.go` | actor user ID มาจาก authenticated principal ห้าม client override; missing preference อ่านเป็น dynamic `all`; subset ต้องมี 1–256 unique locale IDs, ผ่าน body cap 32 KiB และเลือกได้เฉพาะ locale ที่ enabled+user_selectable; all mode คืนทุก locale ที่เข้าเงื่อนไขโดยไม่ materialize rows |
| L5A `[TDD]` preference persistence | `api/internal/modules/localization/adapters/gorm/preference_repository_test.go` | `api/internal/modules/localization/adapters/gorm/preference_repository.go` | transaction เดียว lock header + bulk lock/revalidate locale rows ก่อน transactional optimistic CAS: missing+expected 0 insert v1 โดย concurrent loser conflict, replace subset แบบ exact set, เปลี่ยนเป็น all ต้องลบ child rows, เปลี่ยนกลับ subset ต้อง version เดินต่อเนื่องกัน ABA; stale/mismatch rollback ทุก side effect, ไม่มี generic hard-delete header; read subset join/filter current enabled+user_selectable แบบ deterministic และถ้า effective subset ว่างให้ rendering fallback system default โดยไม่ mutate/widen เป็น all |
| L6 `[TDD]` system-only edits | `api/internal/modules/localization/application/catalog/update_test.go` | `api/internal/modules/localization/application/catalog/update.go` | ต้องมี `localization.translations.update.system`; account role ไม่ผ่าน, application กำหนด write provenance `editor|import` เองและ client ห้ามส่ง/override source |
