...
| M11 | `api/tests/migrations/role_permissions_test.go` | `api/internal/modules/authorization/migrations/{dialect}/000003_create_role_permissions.sql` | role/permission unique pair; role FK cascade, permission FK restrict เพื่อรักษา dormant mapping และ reverse `(permission_id, role_id)` index สำหรับ bounded invalidation |
| M12 | `api/tests/migrations/membership_roles_test.go` | `api/internal/modules/authorization/migrations/{dialect}/000004_create_membership_roles.sql` | account + membership + role unique tuple, composite account-consistency FKs และ reverse `(role_id, account_id, membership_id)` index |
